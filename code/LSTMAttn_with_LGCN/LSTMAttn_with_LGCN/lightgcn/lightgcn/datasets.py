...
def prepare_dataset(device, basepath, verbose=True, logger=None):
    ...
    train_data, valid_data, _, _ = train_test_split(train_data_all, train_data_all['assessmentItemID'],
                 test_size=0.3, shuffle=True, stratify=train_data_all['assessmentItemID'], random_state=34)

    if train_data_all.userID.nunique() == train_data.userID.nunique() and train_data_all.assessmentItemID.nunique() == train_data.assessmentItemID.nunique() and train_data_all.userID.nunique() == valid_data.userID.nunique() and train_data_all.assessmentItemID.nunique() == valid_data.assessmentItemID.nunique():
        print("Node Set okay!!!")
    else:
        print("Wrong Node Set!!!!!!!!!")

    id2index = indexing_data(data)
    with open('id2index.pickle','wb') as fw:
